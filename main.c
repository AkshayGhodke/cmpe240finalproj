...
unsigned char	roomTempF;
unsigned int metricUnits = 1; //C is default
char str[SPRINTF_SIZE];
							
char securityCode[SECURITY_CODE];					// Buffer for security code

unsigned int countDigit;					// Count for the digits entered in code 
...
// C - F
unsigned char celciusToFarenheit(unsigned char roomTemp){
	return (unsigned char)((int)roomTemp * 1.8 + 33);
}

// Display '*' for each digit entered
void confScreen(countDigit){
						switch(countDigit)
						{
							case '1':
								sprintf(str, '*');
								displayText("000000", "FFFFFF", 6, str, 50, 50);
								break;
							case '2':
								sprintf(str, "*	*");
								displayText("000000", "FFFFFF", 6, str, 50, 50);
...
void printTempF(void){
						roomTemp = readOneByteFromSlave(ROOM_TEMP);
						roomTempF = celciusToFarenheit(roomTemp);
						sprintf(str, "%-5bu F", roomTempF);	
						displayText("000000", "FFFFFF", 5, str,  270, 215);
}

void printTempC(void){
						roomTemp = readOneByteFromSlave(ROOM_TEMP);
						sprintf(str, "%-5bu C", roomTemp);
						displayText("000000", "FFFFFF", 5, str,  270, 215);
...
{	
	...
	if(screen == MAIN_PAGE)														// Main screen
	{
		...
		else																	// Not a command, empty buffer with nullchar
		{
			i = 0;
		...
		if(userCommand[0] == '(')
		{
			switch (userCommand[1])
			{
				...
				case 'A':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'0');
					}
					break;
					// Call a function here	or do something here
				case 'B':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'1');
					}
					else
					{
						//displayMessage();			// display error message
					}
					break;
				case 'C':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'2');
					}
					break;
				case 'D':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'3');
					}
					break;
				case 'E':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'4');
					}
					break;
				case 'F':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'5');
					}
					break;
				case 'G':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'6');
					}
					break;
				case 'H':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'7');
					}
					break;
				case 'I':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'8');
					}
					break;
				case 'J':
					if(	countDigit != 4)
					{
					countDigit += sprintf(securityCode + countDigit,'9');
					}
					break;
				...
				case 'Y':
					...
					else
					{
						countDigit = 0;
						sprintf(securityCode,'0');
						//displayMessage();							// To display Message >> "Wrong Code" in a text window
					}
					break;
					// Call a function here	or do something here
				default:
		...
		if(userCommand[0] == '(')
		{
			switch (userCommand[1])
			{
				...
				//while(1){
							...
							roomTempF = celciusToFarenheit(roomTemp);
							sprintf(str, "%-5bu F", roomTempF);	
							displayText("000000", "FFFFFF", 5, str, 255, 215);
				//}
					// Call a function here	or do something here			
					break;
				case 'B':
...
void main()
{
	...
	while(1)
	{
		if(screen == MAIN_PAGE && metricUnits == 1){
				printTempC();
		}else if(screen == MAIN_PAGE && metricUnits == 0){
				printTempF();
		}
